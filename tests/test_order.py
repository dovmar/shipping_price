...
def test_init_price_sets_price_and_reduced_price(shipping_options: ShippingOptions) -> None:
    order = Order(
...
def test_init_price_raises_for_unknown_provider(shipping_options: ShippingOptions) -> None:
    order = Order(
        order_date=date(2015, 2, 1),
        provider="XX",
        package_size="S",
